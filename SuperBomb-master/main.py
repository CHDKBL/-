...
players = []
bombs = None
(x, y, w) = PropertiesPanel.showHelp(screen)  # 返回帮助界面其实位置和宽度。
clock = pygame.time.Clock()
clock.tick(60)
while True:
    # 暂停后事件处理就停止，但必须到里面的pygame.key.get_pressed()之后再停止，否则会卡死
    handle_event(players, bombs, is_pause, is_mainmenu)
    if is_mainmenu or is_restart:  # 判断是否是主界面或重开，如果重开则直接到地图生成
        # 背景
        ...
        if is_restart or btn_start.render():  # 判断是否按下开始或重开
            is_mainmenu = False
            if is_restart:
                is_restart = False
            is_pause = False
            ...
            PropertiesPanel.draw_items(screen)  # 绘制右侧状态栏
        ...
        for player in players:
            if player:
                player.show()
        is_pause, is_mainmenu, is_restart = Button.btn_deal(screen=screen, is_pause=is_pause, btn_pause=btn_pause,
                                                         btn_continue=btn_continue, btn_backmenu=btn_backmenu, btn_restart=btn_restart)
        if len(players) == 1:  # 判断哪位玩家胜利
            is_mainmenu = True
            if players[0].id == 1:
