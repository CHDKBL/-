    ...
    def BuildMap(self, screen):
        ...
        for pair in boxs:
            block = self.Paste(pair, "resources/image/box" +
                               str(random.randint(0, 3)) + ".png", BOX)  # 随机0-3，包含头尾
            self.blocks.add(block)

        for col in range(2, 19, 4):  # 2-19不包括19，每次+4
            for row in range(2, 19, 4):
                barriers.append((col, row))  # 固定方块
